            ...
            steps {
                script {
                    ...
                    if (fileExists(filePath)) {
                        ...
                        nexusArtifactUploader artifacts: [
                            [
                                artifactId: "${NEXUS_REPOSITORY}", // Use the repository name as the artifactId
                                classifier: '', 
                                file: "${filePath}/build.tar.gz", // Adjust the file path
                                type: 'tar.gz'
                            ]
                        ], 
                        credentialsId: NEXUS_CREDENTIAL_ID, 
                        groupId: '', // Leave groupId empty or provide the appropriate value
                        nexusUrl: NEXUS_URL, 
                        nexusVersion: NEXUS_VERSION, 
                        protocol: NEXUS_PROTOCOL, 
                        repository: "${NEXUS_REPOSITORY}", // Use the repository name here
                        version: "${ARTIFACT_VERSION}"
                    } else {
                        error "File 'build' does not exist."
                    }
